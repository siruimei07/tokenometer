...
| ChatGPT | 从官方 `conversations.json` 当前可见分支的用户/助手文本生成会话、提示、每日估算，以及导出中存在的模型标签（可能缺失） | 用户在“设置”中手动导入；不是实时数据 | 精确 token、缓存、额度、费用、设备、工具调用与账单模型 |
...
- **总览**：今日/累计摘要、Codex 额度、逐设备同步状态、活动热图和短期趋势；部分损坏、超大或不可读记录会明确提示并保留其余数据。
